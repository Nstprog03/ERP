...
{
  unset ($_SESSION["farmer_filter_data"]);
  unset ($_SESSION["farmer_filter_selected"]);
  ...
  $isFilter=false;
  $selectedArr=array();
  if (isset($_POST['filter']) || isset($_SESSION['farmer_filter_data'])) 
  {

    $isFilter=true;
    $main_query="select * from farmer where";
    $where_cond = array();
    ...
    if(isset($_POST['filter']))
    {
      ...
      if(isset($_POST['farmer'])){
        ...
        $where_cond[] = " id in (".$farmer.")";
      }

      if(isset($_POST['account_number'])){
        $where_cond[] = " ac_no like '%".$_POST['account_number']."%'";
      }

      $selectedArr=$_POST;
      $_SESSION['farmer_filter_selected']=$selectedArr;
      $_SESSION['farmer_filter_data']=$where_cond;
    }
    else
    {
      $where_cond=$_SESSION['farmer_filter_data'];
      $selectedArr=$_SESSION['farmer_filter_selected'];
    }
    ...
    if(!empty($where_cond)){
      ...
      $sql = "select * from farmer ORDER BY id DESC LIMIT $start_from, $per_page_record";     
      $result = mysqli_query($conn, $sql);
    }
  ...
  <body>
      ...
      <div id="sidebarnav"></div>
        ...
        <div class="container-fluid">
          <a class="navbar-brand" href="index.php"><span class="page-name-top"><span class="icon-report_dashboard"></span> Farmer Database</span></a>
        ...
        <!-- last change on table START-->
          <?php
          $sqlLastChange="select username,updated_at from farmer order by updated_at DESC LIMIT 1";
           ...
           <div class="card viewfilter">
              <div class="card-header">Filter</div>
                <div class="card-body"> 
                    <form action="" method="post" enctype="multipart/form-data">
                      <div class="row"> 
                          ...
                          <select name="farmer[]" data-live-search="true" class="form-control searchDropdown" title="Select" multiple="">
                            <?php                   
                              foreach ($conn->query($party_sql) as $party_result){ 
                                if(isset($selectedArr['farmer']) && in_array($party_result['id'], $selectedArr['farmer'])){ 
                                  echo "<option  value='".$party_result['id']."' selected>" .$party_result['farmer_name']. "</option>";
                                }else{
                                    echo "<option  value='".$party_result['id']."'>" .$party_result['farmer_name']. "</option>";
                                }
                              }
                            ?>                              
                          </select>
                        ...
                        <div class="form-group col-md-4">
                          <label for="farmer">Account number</label>
                          <input type="text" name="account_number" value="<?= isset($selectedArr['account_number'])?$selectedArr['account_number']:''; ?>" class='form-control'>
                        </div>
                      </div>                    
                      <div class="row">
                ...
                <div class="card mt-3">
                      ...
                      <table id="example" class="table table-striped table-bordered" style="width:100%">
                        ...
                        <tbody>
                          ...
                          <tr>
                           ...
                           <td><?php echo $row['vlg_name'] ?></td>
                           <td><?php echo $row['tal_name'] ?></td>
                    ...
                    <?php if (!isset($_SESSION['farmer_filter_data'])) {
