...
if(isset($_GET['id']))
{
	...
	// from 
	// seller_conf s, party p, external_party ep
	// where 
	// s.firm=p.id AND 
	// s.external_party=ep.partyname AND
	// s.id='".$id."'";
    ...
    if($partyrow['logo_img']!='')
    {
    	$logo_image="../firm/files/logo/".$partyrow['logo_img'];
        $img_extension=substr($partyrow['logo_img'], strpos($partyrow['logo_img'], ".") + 1);
    }
    else
    {
    ...
    if($partyrow['stamp_img']!='')
    {
    	$stamp_image="../firm/files/stamp/".$partyrow['stamp_img'];
        $stamp_img_extension=substr($partyrow['stamp_img'], strpos($partyrow['stamp_img'], ".") + 1);
    }
    else
    {
...
//------------header section END---------------------
	...
	$pdf->Text(20, 63,$partyrow['party_name']);
	$pdf->setFont('Arial');
	$pdf->setFontSize(10);
	$pdf->Text(20, 69,'GST No. '.$partyrow['gst_in']);
	$pdf->SetLineWidth(0.5);
	$pdf->Line(100, 50, 100, 75);
	$pdf->setFont('Arial','B',12);
	$pdf->Text(105, 55,"Sales Order No.  : ");
	$pdf->setFont('Arial');
	$pdf->Text(145, 55,$row['sales_conf']);
	$sales_date='';
	if($row['sales_date']!='' && $row['sales_date']!='0000-00-00')
	{
	...
	$bill = "select * from external_party where id='".$row['external_party']."'";
  $billresult = mysqli_query($conn, $bill);
  $billrow = mysqli_fetch_assoc($billresult);
	...
	$pdf->setFont('Arial','B',12);
	$pdf->Text(33, 93,$billrow['partyname']);
	$pdf->setFont('Arial');
	$pdf->setFontSize(10);
	$pdf->Text(33, 99,$billrow['address'].', '.$billrow['city'].' - '.$billrow['pincode']);
	$pdf->setFont('Arial');
	$pdf->setFontSize(10);
	$pdf->Text(33, 105,'GST No. : '.$billrow['gstin']);
  ...
  if(isset($products_row))
  {
	...
	$pdf->Cell(45,8,$row['cont_quantity'],0,1,'L');
	$pdf->SetXY(12,155);
	$pdf->Cell(45,8,'Rate In Rs.(Candy) : ',0,0,'L');
	$pdf->Cell(45,8,$row['candy_rate'],0,1,'L');
	//product variety
	...
	$pdf->setFont('Arial','B',12);
	$pdf->Text(132, 290,$partyrow['party_name']);
	$pdf->output();
	//save pdf in folder


	include('../global_function.php'); 
    $data=getFileStoragePath("sales_conf",$_SESSION['sales_financial_year_id']);  //function from global_function file
	...
	$filename="SC_".$row['sales_conf'].'_'.$timestamp.'.pdf';
   ...
   $sql_pdf="insert into pdf (table_indicator,record_id,file_name,username,created_at) value('1','".$_GET['id']."','".$filename."','".$_SESSION['username']."','".$timestamp2."')";
