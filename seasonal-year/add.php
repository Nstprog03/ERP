...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($result) > 0) 
    {
     ...
     echo $errorMsg = '<h2>Entered Seasonal Year is already Exists</h2>';
     exit;

    }
    else {
     $sql = "INSERT INTO seasonal_year(startdate, enddate, username,created_at,updated_at) VALUES ('".$startdate."', '".$enddate."', '".$username."', '".$timestamp."', '".$timestamp."')";
        $result = mysqli_query($conn, $sql);
        if($result){
