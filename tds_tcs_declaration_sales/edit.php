<?php
   ...
   if(isset($_GET['module']))
   {
     ...
     if (isset($_GET['id'])) {
       ...
       if(!isset($errorMsg))
       {
         $sql = "UPDATE `tds_tcs_declaration` SET 
         `firm_id`='".$firm."',
         `financiyal_year_id`='".$financiyal_year_id."',
         `module_indicator`='".$module."',
         ...
         <div id="content">
            ...
            <nav class="navbar navbar-expand-md navbar-light navbar-laravel">
               <div class="container-fluid">
                  <a class="navbar-brand" href="index.php?module=<?= $module ?>"><span class="page-name-top"><span class="icon-report_dashboard"></span> Edit TDS/TCS Declaration</span></a>
                              ...
                              <div class="col-md-4">
                                 <div class="form-group">
                                    ...
                                    <?php                   
                                       ...
                                       {
                                           echo "<option "; if($row['firm_id'] == $result['id']){ echo "selected";} echo" value='".$result['id']."'>".$result['party_name']. "</option>";
                                       }
                                       ?>                              
                                    </select>
                                 </div>
                                    ...
                                    <?php                   
                                       ...
                                       for($x = 0; $x < $arrlength; $x++) {
                                         ...
                                         //current financial year selected
                                               ...
                                               {
                                               echo "<option "; if($row['financiyal_year_id'] == $storeDates[$x]['id']){ echo "selected"; } echo " value=" .$storeDates[$x]['id']." >".date('Y',strtotime($storeDates[$x]['startdate']))."-".date('Y',strtotime($storeDates[$x]['enddate']))."</option>";
                                               }else{
                                               echo "<option "; if($row['financiyal_year_id'] == $storeDates[$x]['id']){ echo "selected"; } echo " value=" .$storeDates[$x]['id'].">".date('Y',strtotime($storeDates[$x]['startdate']))."-".date('Y',strtotime($storeDates[$x]['enddate']))."</option>";
                                               }
                                       }
                              ...
                              <div class="form-group col-md-4">
                                <label for="status">Select Turnover</label>                  
                                 <select class="form-control" name="turnover" required="">
                                    <option <?php if($row['turnover'] == "Rs.10 crores"){ echo "selected";} ?> value="Rs.10 crores">Rs.10 crores</option>
                                    <option <?php if($row['turnover'] == "Less 10 crore"){ echo "selected";} ?> value="Less 10 crore">Less 10 crore</option>
                                 </select>
                              </div>
                              <div class="form-group col-md-4">
                                 <label for="audit_report_id">Select Audit & ITR</label>
                                 <?php
                                    $sql = "select * from party_audit_report where party_name = '".$row['firm_id']."' AND ad_report_type='IT Return Reoport'";
                                     $result = mysqli_query($conn, $sql);
